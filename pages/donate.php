...
if($settings[2]->value == 0 && !$user->isLoggedIn()){
    ...
    if($settings[3]->value == 1){
        ...
        foreach($latest_donors as $latest_donor){
            ...
            $latest_donors_string .= '<p><div class="row vertical-align"><div class="col-md-3"><a href="/profile/' . htmlspecialchars($latest_donor->ign) . '"><img class="img-rounded" src="https://minotar.net/helm/' . ((strlen($latest_donor->ign) > 0 && $latest_donor->ign != '----') ? htmlspecialchars($latest_donor->ign) : htmlspecialchars($latest_donor->ign)) . '/30.png" /></a></div>';
