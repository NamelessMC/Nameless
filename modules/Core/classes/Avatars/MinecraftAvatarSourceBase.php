 ...
 * @license MIT
 */
abstract class MinecraftAvatarSourceBase {
    ...
    protected bool $_supports_usernames = false;
    ...
    public function supportsUsernames(): bool {
        return $this->_supports_usernames;
    }
     ...
     *
     * @param string $identifier UUID of avatar to get.
     ...
     */
    public function getAvatar(string $identifier, string $perspective, int $size = 128): string {
        return $this->formatUrl($this->getUrlToFormat($perspective), $identifier, $size);
    }

    /**
     * Replace placeholders in raw url with identifier and size of requested avatar.
     *
     * @param string $url_to_format Raw url to replace placeholders in.
     * @param string $identifier Identifier (UUID, or username) of avatar to get.
     * @param int $size Size of avatar image in pixels to get.
     *
     * @return string Formatted url.
     */
    public function formatUrl(string $url_to_format, string $identifier, int $size): string {
        return str_replace(
            ['{identifier}', '{size}'],
            [$identifier, $size],
            $url_to_format
