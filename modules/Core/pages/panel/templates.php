...
if (!isset($_GET['action'])) {
    ...
    foreach ($templates as $item) {
        ...
        if (in_array($item->name, $loaded_templates, true)) {
    ...
    if ($cache->hasCashedData('all_templates')) {
    ...
    switch ($_GET['action']) {
        ...
        case 'settings':
            ...
            if (($template instanceof TemplateBase) && $template->getSettings()) {
                ...
                try {
                    if (Token::check()) {
                        ...
                        foreach (Group::all() as $group) {
                            ...
                            if (count($perm_query)) {
                                foreach ($perm_query as $query) {
                                    if ($query->group_id === (string)$group->id) {
                                        $perm_exists = 1;
                                        $update_id = $query->id;
                                        break;
                                    }
