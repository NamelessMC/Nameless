...
if (isset($_GET['user'])) {
        ...
        try {
            if (Token::checK()) {
                ...
                if (!$user->hasPermission('modcp.punishments.revoke') || !count($infraction) || ($infraction[0]->punished !== $query->id)) {
                    Redirect::to(URL::build('/panel/users/punishments/', 'user=' . urlencode((string)$query->id)));
                }
                ...
                if (isset($_POST['type'])) {
                    ...
                    if (isset($_POST['reason']) && strlen($_POST['reason']) >= 5 && strlen($_POST['reason']) <= 5000) {
                        try {
                            ...
                            $banned_user = new User((string)$query->id);
                                        ...
                                        if ($users->count()) {
                                            ...
                                            foreach ($users as $item) {
                                                ...
                                                Alert::create(
                                                    ...
                                                    URL::build('/panel/users/punishments/', 'user=' . urlencode((string)$query->id))
                                                );
                                            }
                                        }
                                    }
        ...
        foreach ($previous_punishments as $punishment) {
            ...
            try {
                $previous_punishments_array[] = [
                    ...
                    'date_friendly' => ($punishment->created ? $time_ago->inWords($punishment->created, $language) : $time_ago->inWords($punishment->infraction_date, $language)),
                    'revoke_link' => (($user->hasPermission('modcp.punishments.revoke') && $punishment->type !== '4') ? URL::build('/panel/users/punishments/', 'user=' . urlencode((string)$query->id) . '&do=revoke&id=' . urlencode($punishment->id)) : 'none'),
    ...
    try {
        $smarty->assign([
            ...
            'BACK_LINK' => URL::build('/panel/user/' . urlencode((string)$view_user->data()->id)),
    ...
    if (isset($_POST['username']) && Input::exists()) {
        try {
            if (Token::check()) {
        ...
        foreach ($results->data as $result) {
            ...
            try {
                $smarty_results[] = [
                    ...
                    'time' => ($result->created ? $time_ago->inWords($result->created, $language) : $time_ago->inWords($result->infraction_date, $language)),
