            ...
            if ($validation->passed()) {
                ...
                if ($user_query->exists()) {
                        ...
                        if ($user->checkCredentials($username, Input::get('password'), $method_field)) {
                            if (!isset($_POST['tfa_code'])) {
                                ...
                                if ($user_query->data()->tfa_type == 1) {
                                    // App
                                    $tfa = new \RobThree\Auth\TwoFactorAuth(new \RobThree\Auth\Providers\Qr\QRServerProvider(), 'NamelessMC');
                    ...
                    if (!isset($return_error)) {
                        ...
                        if (Settings::get('mc_integration') && Settings::get('authme') && DB::getInstance()->get('users', ['id', $user_id])->first()->authme_sync_password) {
                            try {
