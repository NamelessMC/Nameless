    ...
    try {
        if (Token::check()) {
            ...
            if ($latest_forums_query->count()) {
                $latest_forums = array_map(static fn($latest_forum) => $latest_forum->id, $latest_forums_query->results());
            }
            ...
            if ($latest_topics_query->count()) {
                $latest_topics = array_map(static fn($latest_topic) => $latest_topic->id, $latest_topics_query->results());
            }
