            ...
            <div class="card-body">
              ...
              <form role="form" action="" method="post">
                <div class="form-group">
                  ...
                  <input type="text" name="title" class="form-control" id="InputTitle" placeholder="{$FORUM_TITLE}"
                    value="{$FORUM_TITLE_VALUE}">
                </div>
                ...
                <div class="form-group">
                  <div class="form-group">
                    ...
                    <select class="form-control" id="InputType" name="forum_type">
                      ...
                      <option value="category" {if $FORUM_TYPE_VALUE=='category' } selected{/if}>{$FORUM_TYPE_CATEGORY}
                      </option>
                    </select>
                  </div>
                </div>
                ...
                <div class="form-group custom-control custom-switch">
                  ...
                  <input id="InputDisplay" name="display" type="checkbox" class="custom-control-input" value="1" {if
                    $DISPLAY_TOPICS_AS_NEWS_VALUE} checked{/if} />
                ...
                <div class="form-group custom-control custom-switch">
                  <input id="InputForumRedirect" name="redirect" type="checkbox" class="custom-control-input" value="1"
                    {if $REDIRECT_FORUM_VALUE} checked{/if} />
                ...
                <div class="form-group">
                  ...
                  <input placeholder="{$REDIRECT_URL}" name="redirect_url" id="InputForumRedirectURL" type="text"
                    class="form-control" value="{$REDIRECT_URL_VALUE}" />
                </div>

                <div class="form-group">
                  <label for="InputHooks">{$INCLUDE_IN_HOOK} <span class="badge badge-info" data-toggle="popover"
                      data-title="{$INFO}" data-content="{$HOOK_SELECT_INFO}"><i class="fa fa-question"></i></label>
                  <select name="hooks[]" id="InputHooks" class="form-control" multiple>
                    {foreach from=$HOOKS_ARRAY item=hook}
                    <option value="{$hook.id}" {if in_array($hook.id, $FORUM_HOOKS)} selected {/if}>{$hook.name|ucfirst}
                    </option>
                    {/foreach}
                  </select>
                </div>
                ...
                <div class="form-group">
                  <label for="InputDefaultLabels">{$DEFAULT_LABELS} <span class="badge badge-info" data-toggle="popover"
                      data-title="{$INFO}" data-content="{$DEFAULT_LABELS_INFO}"><i class="fa fa-question"></i></label>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td><input type="hidden" name="perm-view-0" value="0" /><input onclick="colourUpdate(this);"
                            name="perm-view-0" id="Input-view-0" value="1" type="checkbox" {if
                            isset($GUEST_PERMISSIONS->view) && $GUEST_PERMISSIONS->view eq 1} checked{/if} />
                        </td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td><input type="hidden" name="perm-view_others-0" value="0" /><input
                            onclick="colourUpdate(this);" name="perm-view_others-0" id="Input-view_others-0" value="1"
                            type="checkbox" {if isset($GUEST_PERMISSIONS->view_other_topics) &&
                          $GUEST_PERMISSIONS->view_other_topics eq 1} checked{/if} />
                        </td>
                        <td></td>
                      </tr>
                      ...
                      <tr>
                        ...
                        <td><input type="hidden" name="perm-view-{$group->id|escape}" value="0" />
                          <input onclick="colourUpdate(this);" name="perm-view-{$group->id|escape}"
                            id="Input-view-{$group->id|escape}" value="1" type="checkbox" {if isset($group->view) &&
                          $group->view eq 1} checked{/if} />
                        </td>
                        <td><input type="hidden" name="perm-topic-{$group->id|escape}" value="0" /><input
                            onclick="colourUpdate(this);" name="perm-topic-{$group->id|escape}"
                            id="Input-topic-{$group->id|escape}" value="1" type="checkbox" {if
                            isset($group->create_topic) && $group->create_topic eq 1} checked{/if} />
                        </td>
                        <td><input type="hidden" name="perm-edit_topic-{$group->id|escape}" value="0" /><input
                            onclick="colourUpdate(this);" name="perm-edit_topic-{$group->id|escape}"
                            id="Input-edit_topic-{$group->id|escape}" value="1" type="checkbox" {if
                            isset($group->edit_topic) && $group->edit_topic eq 1} checked{/if} />
                        </td>
                        <td><input type="hidden" name="perm-post-{$group->id|escape}" value="0" /><input
                            onclick="colourUpdate(this);" name="perm-post-{$group->id|escape}"
                            id="Input-post-{$group->id|escape}" value="1" type="checkbox" {if isset($group->create_post)
                          && $group->create_post eq 1} checked{/if} />
                        </td>
                        <td><input type="hidden" name="perm-view_others-{$group->id|escape}" value="0" /><input
                            onclick="colourUpdate(this);" name="perm-view_others-{$group->id|escape}"
                            id="Input-view_others-{$group->id|escape}" value="1" type="checkbox" {if
                            isset($group->view_other_topics) && $group->view_other_topics eq 1} checked{/if} />
                        </td>
                        <td><input type="hidden" name="perm-moderate-{$group->id|escape}" value="0" /><input
                            onclick="colourUpdate(this);" name="perm-moderate-{$group->id|escape}"
                            id="Input-moderate-{$group->id|escape}" value="1" type="checkbox" {if
                            isset($group->moderate) && $group->moderate eq 1} checked{/if} />
                        </td>
                      </tr>
                      <script>
