                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            ...
                            <form action="" method="post">
                                ...
                                <div class="form-group">
                                    <label for="InputAddress">{$SERVER_ADDRESS}</label> <span
                                ...
                                <div class="form-group">
                                    <label for="inputPort">{$SERVER_PORT}</label> <span class="badge badge-info"><i
                                            ...
                                            data-content="{$SERVER_PORT_INFO}"></i></span>
                                        ...
                                        {if count($AVAILABLE_PARENT_SERVERS)}
                                            {foreach from=$AVAILABLE_PARENT_SERVERS item=available_server}
                                                {if $available_server->id neq $SERVER_ID}
                                                    <option value="{$available_server->id|escape}"
                                                        {if $PARENT_SERVER_VALUE eq $available_server->id} selected{/if}>
                                                        {$available_server->name|escape}</option>
                                                {/if}
                                            {/foreach}
                                        {/if}
                                    </select>
                                ...
                                <div class="form-group custom-control custom-switch">
                                    ...
                                    <input id="inputBungeeInstance" name="bungee_instance" type="checkbox"
                                        class="custom-control-input" value="1" {if $BUNGEE_INSTANCE_VALUE}
                                        checked{/if} />
                                    <label for="inputBungeeInstance" class="custom-control-label">
                                        {$BUNGEE_INSTANCE} <span class="badge badge-info"><i
                                ...
                                <div class="form-group custom-control custom-switch">
                                    ...
                                    <input id="inputBedrock" name="bedrock" type="checkbox" class="custom-control-input"
                                        value="1" {if $BEDROCK_VALUE} checked{/if} />
                                    <label for="inputBedrock" class="custom-control-label">
                                        {$BEDROCK} <span class="badge badge-info"><i class="fa fa-question-circle"
                                                data-container="body" data-toggle="popover" data-placement="top"
                                                title="{$INFO}" data-content="{$BEDROCK_INFO}"></i></span>
                                    </label>
                                </div>

                                <div class="form-group custom-control custom-switch">
                                    <input type="hidden" name="pre_17" value="0">
                                    <input id="inputPre17" name="pre_17" type="checkbox" class="custom-control-input"
                                        value="1" {if $PRE_17_VALUE} checked{/if} />
                                ...
                                <div class="form-group custom-control custom-switch">
                                    ...
                                    <input id="inputStatusQueryEnabled" name="status_query_enabled" type="checkbox"
                                        class="custom-control-input" value="1" {if $ENABLE_STATUS_QUERY_VALUE}
                                        checked{/if} />
                                    <label for="inputStatusQueryEnabled" class="custom-control-label">
                                        {$ENABLE_STATUS_QUERY} <span class="badge badge-info"><i
                                                class="fa fa-question-circle" data-container="body"
                                                data-toggle="popover" data-placement="top" data-html="true"
                                                title="{$INFO}" data-content="{$ENABLE_STATUS_QUERY_INFO}"></i></span>
                                    </label>
                                </div>
                                <div class="form-group custom-control custom-switch">
                                    <input type="hidden" name="show_ip_enabled" value="0">
                                    <input id="inputShowIPOnStatus" name="show_ip_enabled" type="checkbox"
                                        class="custom-control-input" value="1" {if $SHOW_IP_ON_STATUS_PAGE_VALUE}
                                        checked{/if} />
                                    <label for="inputShowIPOnStatus" class="custom-control-label">
                                        {$SHOW_IP_ON_STATUS_PAGE} <span class="badge badge-info"><i
                                                class="fa fa-question-circle" data-container="body"
                                                data-toggle="popover" data-placement="top" data-html="true"
                                                title="{$INFO}"
                                ...
                                <div class="form-group custom-control custom-switch">
                                    ...
                                    <input id="inputQueryEnabled" name="query_enabled" type="checkbox"
                                        class="custom-control-input" value="1" {if $ENABLE_PLAYER_LIST_VALUE}
                                        checked{/if} />
                                    <label for="inputQueryEnabled" class="custom-control-label">
                                        {$ENABLE_PLAYER_LIST} <span class="badge badge-info"><i
                                                class="fa fa-question-circle" data-container="body"
                                                data-toggle="popover" data-placement="top" data-html="true"
                                                title="{$INFO}" data-content="{$ENABLE_PLAYER_LIST_INFO}"></i></span>
                                    </label>
                                </div>
                                <div class="form-group">
                                    <label for="inputQueryPort">{$SERVER_QUERY_PORT}</label> <span
                                        class="badge badge-info"><i class="fa fa-question-circle" data-container="body"
