            ...
            <div class="card-body">
              ...
              <form action="" method="post">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="page_title" id="inputTitle" placeholder="{$PAGE_TITLE}"
                    value="{$PAGE_TITLE_VALUE}">
                </div>
                <div class="form-group">
                  <label for="inputURL">{$PAGE_PATH}</label>
                  <input type="text" class="form-control" name="page_url" id="inputURL" placeholder="{$PAGE_PATH}"
                    value="{$PAGE_PATH_VALUE}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group custom-control custom-switch">
                  <input id="inputBasicPage" name="basic" type="checkbox" class="custom-control-input" {if
                    $BASIC_PAGE_VALUE eq 1} checked{/if} />
                  <label class="custom-control-label" for="inputBasicPage">{$BASIC_PAGE}</label>
                </div>
                <div class="form-group custom-control custom-switch">
                  <input id="inputRedirect" name="redirect_page" type="checkbox" class="custom-control-input" {if
                ...
                <div class="form-group custom-control custom-switch">
                  <input id="inputTarget" name="target" type="checkbox" class="custom-control-input" {if $TARGET_VALUE
                    eq 1} checked{/if} />
                  <label class="custom-control-label" for="inputTarget">{$TARGET}</label>
                </div>
                <div class="form-group custom-control custom-switch">
                  <input id="inputUnsafeHTML" name="unsafe_html" type="checkbox" class="custom-control-input" {if
                    $UNSAFE_HTML_VALUE eq 1} checked{/if} />
                  <label class="custom-control-label" for="inputUnsafeHTML">{$UNSAFE_HTML}</label>
                  <span data-toggle="popover" data-content="{$UNSAFE_HTML_WARNING}" class="badge badge-warning"><i
                      class="fas fa-exclamation-triangle"></i></span>
                </div>
                <div class="form-group custom-control custom-switch">
                  <input id="inputSitemap" name="sitemap" type="checkbox" class="custom-control-input" {if
                    $INCLUDE_IN_SITEMAP_VALUE eq 1} checked{/if} />
                  <label class="custom-control-label" for="inputSitemap">{$INCLUDE_IN_SITEMAP}</label>
                </div>
                <hr />
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td><input type="hidden" name="perm-view-0" value="0" /><input onclick="colourUpdate(this);"
                          name="perm-view-0" id="Input-view-0" value="1" type="checkbox" {if $GUEST_PERMS eq 1}
                          checked{/if} /></td>
                    </tr>
                    ...
                    <tr>
                      ...
                      <td><input type="hidden" name="perm-view-{$item.id}" value="0" /><input
                          onclick="colourUpdate(this);" name="perm-view-{$item.id}" id="Input-view-{$item.id}" value="1"
