    ...
    <div class="ui centered three stackable cards" id="servers">
        {foreach from=$SERVERS item=server}
          <div class="ui fluid card center aligned server" style="height: 100%;" id="server{$server->id|escape}"
               data-id="{$server->id|escape}" data-bungee="{$server->bungee|escape}"
               data-bedrock="{$server->bedrock|escape}" data-players="{$server->player_list|escape}">
            <div class="content">
              <div class="header">
                  {if $server->show_ip}
                    <div class="ui top right attached label" data-toggle="popup"
