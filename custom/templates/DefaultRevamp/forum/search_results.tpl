      ...
      <div class="ui padded attached segment">
        <h3 class="ui header">
          ...
          <div class="ui wide popup">
            ...
            <a href="{$result.post_author_profile}" style="{$result.post_author_style}">{$result.post_author}</a> | {$result.post_date_full}
          </div>
          <div class="sub header">
            <a href="{$result.post_author_profile}" style="{$result.post_author_style}" data-poload="{$USER_INFO_URL}{$result.post_author_id}">{$result.post_author}</a> &middot; <span data-toggle="tooltip" data-content="{$result.post_date_full}">{$result.post_date_friendly}</span>
          </div>
        </h3>
        {$result.content}
      </div>
