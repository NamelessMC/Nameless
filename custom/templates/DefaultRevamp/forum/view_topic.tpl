  ...
  <div class="ui container">
    <div class="ui breadcrumb">
        ...
        {foreach from=$BREADCRUMBS item=breadcrumb}
          ...
          <a class="section{if isset($breadcrumb.active)} active{/if}"
    ...
    <h2 class="ui header">
      <div
        class="topic title">{if count($TOPIC_LABELS)}{foreach from=$TOPIC_LABELS item=label}{$label} {/foreach}{/if}{$TOPIC_TITLE}</div>
                  ...
                  <div class="ui list">
                    ...
                    <div class="item">
                      <div class="content">
                        ...
                        <div
                          class="res right floated description">{$reply.user_topics_count|regex_replace:'/[^0-9]+/':''}</div>
                      </div>
                    </div>
                    <div class="item">
                      <div class="content">
                        ...
                        <div
                          class="res right floated description">{$reply.user_posts_count|regex_replace:'/[^0-9]+/':''}</div>
                      </div>
                    </div>
                  </div>
                                ...
                                {if isset($REACTIONS) && count($REACTIONS)}
                                    {foreach from=$REACTIONS item=reaction}
                                      <form action="{$REACTIONS_URL}" method="post">
                                        ...
                                        <button type="submit"
                                                class="ui mini primary icon button">{$reaction->html}</button>
                                      </form>
                                    {/foreach}
                                {/if}
                            </div>
          ...
          <div class="ui bottom attached secondary segment" id="post-meta">
            ...
            <div class="res right floated">
                ...
                {if isset($reply.buttons.edit)}
                  <a class="ui mini icon button" href="{$reply.buttons.edit.URL}"
                     data-tooltip="{$reply.buttons.edit.TEXT}" data-variation="mini" data-inverted=""><i
                ...
                {if isset($reply.buttons.quote)}
                  <button class="ui mini icon button" onclick="quote({$reply.id})"
                          data-tooltip="{$reply.buttons.quote.TEXT}" data-variation="mini" data-inverted=""><i
                      class="quote left icon"></i></button>
                {/if}
            </div>
          </div>
        </div>
            ...
            <div class="ui small modal" id="modal-report-{$reply.id}">
              ...
              <div class="actions">
                ...
                <a class="ui positive button"
                   onclick="$('#form-report-{$reply.id}').submit();">{$reply.buttons.report.TEXT}</a>
              </div>
            </div>
          {/if}
          {if isset($CAN_MODERATE)}
            ...
            <div class="ui small modal" id="modal-delete-{$reply.id}">
              ...
              <div class="actions">
                <a class="ui negative button">{$CANCEL}</a>
                <a class="ui positive button"
                   onclick="$('#form-delete-{$reply.id}').submit();">{$reply.buttons.delete.TEXT}</a>
              </div>
            </div>
          {/if}
      {/foreach}
