      ...
      {foreach from=$NAV_LINKS key=name item=item}
          ...
          {else}
            <a class="item{if isset($item.active)} active{/if}" href="{$item.link}"
               target="{$item.target}">{$item.icon} {$item.title}</a>
          {/if}
      {/foreach}
  </div>
              ...
              {if isset($MINECRAFT) && isset($SERVER_QUERY)}
                <div class="connect-server">
                    ...
                    {if isset($CLICK_TO_COPY_TOOLTIP)}
                      <h4 class="ui header">
                        <span onclick="copy('#ip')" data-tooltip="{$CLICK_TO_COPY_TOOLTIP}" data-variation="mini"
                              data-inverted="" data-position="bottom center">{$CONNECT_WITH}</span>
                      </h4>
                    {/if}
                </div>
              {/if}
