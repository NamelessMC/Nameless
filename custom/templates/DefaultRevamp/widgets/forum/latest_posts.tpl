                ...
                <div class="ui wide popup">
                  <h4 class="ui header">{$post.topic_title}</h4>
                    {$BY|capitalize} <a href="{$post.last_reply_profile_link}"
                                        style="{$post.last_reply_style}">{$post.last_reply_username}</a>
                  | {$post.last_reply}
                </div>
                <a href="{$post.last_reply_profile_link}" style="{$post.last_reply_style}"
                   data-poload="{$USER_INFO_URL}{$post.last_reply_user_id}">{$post.last_reply_username}</a> &middot;
                <span data-toggle="tooltip" data-content="{$post.last_reply}">{$post.last_reply_rough}</span>
              </div>
            </div>
          </div>
        {/foreach}
