				...
				<div class="ui six wide tablet four wide computer column">
					<div class="ui vertical pointing menu">
                        {foreach from=$LEADERBOARD_PLACEHOLDERS item=placeholder}
							<a class="item leaderboard_tab" name="{$placeholder->safe_name}"
				...
				<div class="ui ten wide tablet twelve wide computer column">
                    {foreach from=$LEADERBOARD_PLACEHOLDERS item=placeholder}
						<div class="leaderboard_table"
							 id="table-{$placeholder->safe_name}-server-{$placeholder->server_id}"
							 style="display: none;">
							...
							<table class="ui fixed single line selectable unstackable small padded res table">
                                ...
                                {foreach from=$LEADERBOARD_PLACEHOLDERS_DATA item=data}
                                    {if $data->name eq $placeholder->name and $data->server_id eq $placeholder->server_id}
										<tr>
											<td>
												<img class="ui middle aligned" src="{$data->avatar}"
													 alt="{$data->username}">
