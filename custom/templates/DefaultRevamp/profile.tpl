                ...
                <a class="ui tiny default icon button" href="{$MESSAGE_LINK}"><i class="envelope icon"></i></a>
                  {if isset($RESET_PROFILE_BANNER)}
                    <form action="{$RESET_PROFILE_BANNER_LINK}" method="post" style="display: inline">
                      ...
                      <button class="ui tiny negative icon button" data-toggle="tooltip"
                              data-content="{$RESET_PROFILE_BANNER}">
                              ...
                              {if isset($LOGGED_IN_USER)}
                                  {if $post.user_id ne $VIEWER_ID}
                                    <a href="{if $post.reactions_link !== "#"}{$post.reactions_link}{else}#{/if}"
                                       data-toggle="popup">Like {if ($post.reactions.count|regex_replace:'/[^0-9]+/':'' != 0)}({$post.reactions.count|regex_replace:'/[^0-9]+/':''}){/if}</a>
                                  {/if}
                                <a data-toggle="modal"
                                   data-target="#modal-reply-{$post.id}">{$REPLY} {if ($post.replies.count|regex_replace:'/[^0-9]+/':'' != 0)}({$post.replies.count|regex_replace:'/[^0-9]+/':''}){/if}</a>
                              {/if}
                            ...
                            <div class="comments">
                                {foreach from=$post.replies.replies item=item}
                                  <div class="comment">
                                    ...
                                    <div class="content">
                                      <a class="author" href="{$item.profile}"
                                         style="{$item.style}">{$item.nickname}</a>
                                      <div class="metadata">
                                        <span class="date" data-toggle="tooltip"
              ...
              <form action="" class="ui form" name="updateBanner" method="post" style="display:inline;">
                <select name="banner" class="image-picker show-html">
                    {foreach from=$BANNERS item=banner}
                      <option data-img-src="{$banner.src}"
                              value="{$banner.name}"{if $banner.active == true} selected{/if}>{$banner.name}</option>
                    {/foreach}
                </select>
                <input type="hidden" name="token" value="{$TOKEN}">
