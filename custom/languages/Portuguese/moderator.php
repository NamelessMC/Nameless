<?php
/*
 *  Made by Samerton
 *  https://github.com/NamelessMC/Nameless/
 *  NamelessMC version 2.0.0-pr12
 *
 *  License: MIT
 *
 *  Portuguese Language - Moderator terms
 *  Translation by Douglas Teles, dasilvaj4 and DeusDrizzyy.
 */

$language = [
    'mod_cp' => 'ModCP',
    'staff_cp' => 'StaffCP',
    'overview' => 'Visão Geral',

    // Spam
    'spam' => 'Spam',
    'mark_as_spam' => 'Marcar como Spam',
    'confirm_spam' => '<p>Tem certeza de que deseja marcar esse usuário como spam?</p><p>O IP do usuário será banido e todo seu conteúdo será removido.</p>',
    'user_marked_as_spam' => 'Usuário marcado como spam com sucesso.',
    'cant_ban_admin' => 'Você não pode banir um administrador!',

    // IP Lookup
    'ip_lookup' => 'Pesquisar IP',
    'search_for_ip' => 'Procurar um nome de usuário ou um endereço IP:',
    'no_ips_with_username' => 'Nenhum endereço IP para esse usuário encontrado.',
    'no_accounts_with_that_ip' => 'Nenhuma conta para esse endereço IP encontrado.',
    '1_account_with_ip' => 'Encontrado 1 conta com o IP {{address}}', // Não substituir {y}.
    'count_accounts_with_ip' => 'Encontrados {{count}} contas com o endereço de IP {{address}}', // Não substituir o {x} ou {y}.
    '1_ip_with_name' => 'Encontrado 1 IP para o usuário {{user}}', // Não substituir {y}.
    'count_ips_with_name' => 'Encontrados {{count}} endereços de IP para o usuário {{user}}', // Não substituir o {x} ou {y}.
    'no_users_or_ips_found' => 'Não foram encontrados usuários ou endereços IP.',

    // Reports
    'reports' => 'Denúncias',
    'report_alert' => 'Nova denúncia enviada.',
    'user_reported' => 'Usuário Denúnciado',
    'comments' => 'Comentários',
    'updated_by' => 'Atualizado Por',
    'actions' => 'Ações',
    'view_closed' => 'Visualizar Fechado',
    'view_open' => 'Visualizar Aberto',
    'viewing_report' => 'Visualizando Denúncia',
    'view_content' => 'Visualizar conteúdo denunciado',
    'no_comments' => 'Sem comentários',
    'new_comment' => 'Novo comentário',
    'report_comment_invalid' => 'Conteúdo do comentário inválido. Certifique-se de que introduziu um comentário entre 1 e 10000 caracteres.',
    'close_report' => 'Fechar denúncia',
    'reopen_report' => 'Reabrir denúncia',
    '1_open_report' => 'Atualmente, há 1 denúncia aberta.',
    'open_reports' => 'Atualmente, há {{count}} denúncias abertas.', // Não substituir {x}.
    'no_open_reports' => 'Atualmente não há denúncias abertas.',
    'no_closed_reports' => 'Atualmente não há denúncias fechadas.',
    'recent_reports' => 'Denúncias Recentes',
    'reported_by' => 'Denunciado por:',
    'website' => 'Website',
    'ingame' => 'Ingame',
    'x_closed_report' => '{{user}} fechar essa denúncia.',
    'x_reopened_report' => '{{user}} reabrir essa denúncia.',
    'report_reopened' => 'Denúncia reaberta com sucesso.',
    'report_closed' => 'Denúncia fechada com sucesso.',
    'comment_created' => 'Comentário adicionado com sucesso.',

    // Punishments
    'punishments' => 'Punições',
    'view_punishments' => 'Ver Punições',
    'banned' => 'Banido',
    'groups' => 'Grupos',
    'punish' => 'Punir',
    'ban' => 'Banir',
    'warn' => 'Advertir',
    'ban_ip' => 'Banir por IP',
    'viewing_user_x' => 'Analisando o usuário {{user}}',
    'previous_punishments' => 'Punições anteriores',
    'no_previous_punishments' => 'Sem punições anteriores',
    'reset_avatar' => 'Redefinir Avatar',
    'warning' => 'Aviso',
    'ip_ban' => 'IP Ban',
    'reason' => 'Motivo',
    'warn_user' => 'Advertir o Usuário',
    'ban_user' => 'Banir o Usuário',
    'enter_valid_punishment_reason' => 'Por favor, digite um motivo válido para a sua punição, deve ter 5 a 5000 caracteres.',
    'user_punished' => 'Usuário punido.',
    'user_punished_alert' => '{{staffUser}} puniu o usuário {{user}}', // Não substituir o {x} (membro da equipe) ou {y} (usuário punido).
    'revoke' => 'Anular',
    'revoked' => 'Anulado',
    'acknowledged' => 'Reconhecimento',
    'confirm_revoke_warning' => 'Você tem certeza que quer anular essa advertência?',
    'confirm_revoke_ban' => 'Você tem certeza que quer anular esse banimento? O usuário vai ser desbanido, mesmo se o usuário tem mais banimentos recentes!',
    'punishment_revoked' => 'Punição anulada.',
    'cant_punish_admin' => 'Você não pode punir um administrador!',
    'viewing_all_punishments' => 'Visualizando todas as punições',
    'no_punishments_found' => 'Nenhuma punição encontrada.',
    'view_user' => 'Visualizando usuário',
    'when' => 'Quando',
    'staff' => 'Staff',
    'type' => 'Tipo',
    'recent_punishments' => 'Punições Recentes',
    'created' => 'Criado:',
    'staff:' => 'Staff:',

    // Users
    'recent_registrations' => 'Cadastros Recentes',
    'reset_profile_banner' => 'Redefinir Banner do Perfil'
];
