<?php
/*
 *  Made by Samerton
 *  https://github.com/NamelessMC/Nameless/
 *  NamelessMC version 2.0.0-pr8
 *
 *  License: MIT
 *
 *  Portuguese Language - Time
 *  Translation By Douglas Teles & dasilvaj4
 *  Last Update: 11/04/2018
 */
$language = [
    /*
     *  Time
     */
    'time' => 'tempo', // Placeholder, no need to translate
    'seconds_short' => 's', // Shortened "seconds", eg "s"
    'less_than_a_minute' => 'a menos de um minuto atrás',
    '1_minute' => '1 minuto atrás',
    '_minutes' => '{{count}} minutos atrás',
    'about_1_hour' => 'cerca de 1 hora atrás',
    '_hours' => '{{count}} horas atrás',
    '1_day' => '1 dia atrás',
    '_days' => '{{count}} dias atrás',
    'about_1_month' => 'cerca de 1 mês atrás',
    '_months' => '{{count}} meses atrás',
    'about_1_year' => 'cerca de 1 ano atrás',
    'over_x_years' => 'mais de {{count}} anos atrás'
];
