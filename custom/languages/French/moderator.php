<?php
/*
 *	Made by White, Ikiae.
 *  https://github.com/NamelessMC/Nameless/
 *  NamelessMC version 2.0.0-pr10
 *
 *  License: MIT
 *
 *  French Language - Users
 */

$language = [
    'staff_cp' => 'Panneau d\'administration',
    'overview' => 'Vue d\'ensemble',

    // Spam
    'spam' => 'Spam',
    'mark_as_spam' => 'Marquer comme Spam',
    'confirm_spam' => '<p>Êtes-vous sûr de vouloir marquer cet utilisateur comme spam?</p><p>L’utilisateur sera banni IP et tout son contenu sera supprimé.</p>',
    'user_marked_as_spam' => 'Utilisateur marqué comme spam avec succès.',
    'cant_ban_admin' => 'Vous ne pouvez pas bannir un administrateur!',

    // IP Lookup
    'ip_lookup' => 'Voir IP',
    'search_for_ip' => 'Recherche d’un nom d’utilisateur ou d’une adresse IP',
    'no_ips_with_username' => 'Aucune adresse IP n’a été trouvée pour cet utilisateur.',
    'no_accounts_with_that_ip' => 'Aucun compte pour cette adresse IP n’a été trouvé.',
    '1_account_with_ip' => 'Trouvé un compte avec l’adresse IP {{address}}', // Don't replace "{y}"
    'count_accounts_with_ip' => 'Trouvé {{count}} compte avec cette adresse IP {{address}}', // Don't replace "{x}" or "{y}"
    '1_ip_with_name' => 'Trouvé une adresse IP pour l\'utilisateur {{user}}', // Don't replace "{y}"
    'count_ips_with_name' => 'Trouvé {{count}} utilisateurs avec cette adresse IP {{user}}', // Don't replace "{x}" or "{y}"
    'no_users_or_ips_found' => 'Pas d’utilisateurs ou d’adresses IP trouvés.',

    // Reports
    'reports' => 'Signalements',
    'report_alert' => 'Nouveau signalement.',
    'user_reported' => 'Utilisateur signalé',
    'comments' => 'Commentaires',
    'updated_by' => 'Mis à jour par',
    'actions' => 'Actions',
    'view_closed' => 'Signalements Clos',
    'view_open' => 'Signalements Ouverts',
    'viewing_report' => 'Regarder les Signalements',
    'view_content' => 'Voir le contenu signalé',
    'no_comments' => 'Aucun commentaire',
    'new_comment' => 'Nouveau commentaire',
    'report_comment_invalid' => 'Contenu des commentaires invalides. Assurez-vous d’avoir entré un commentaire entre 1 et 10 000 caractères.',
    'close_report' => 'Signalements fermés',
    'reopen_report' => 'Réouvrir signalement',
    '1_open_report' => 'Il y a actuellement 1 signalement ouvert.',
    'open_reports' => 'Il y a actuellement {{count}} signalements ouverts.',
    'no_open_reports' => 'Il n\'y a actuellement aucun signalement ouvert.',
    'no_closed_reports' => 'Il n’y a actuellement aucun signalement fermé.',
    'recent_reports' => 'Recent Reports',
    'reported_by' => 'Signalé par:',
    'website' => 'Site web',
    'ingame' => 'En jeux',
    'x_closed_report' => '{{user}} a fermé ce signalement.',
    'x_reopened_report' => '{{user}} a rouvert ce signalement.',
    'report_reopened' => 'Signalement rouvert avec succès.',
    'report_closed' => 'Signalement fermé avec succès.',
    'comment_created' => 'Commentaire créer avec succès.',

    // Punishments
    'punishments' => 'Sanctions',
    'view_punishments' => 'Voir les sanctions',
    'banned' => 'Banni',
    'groups' => 'Groupes',
    'punish' => 'Punir',
    'ban' => 'Ban',
    'warn' => 'Avertissement',
    'ban_ip' => 'Ban IP',
    'viewing_user_x' => 'Voir l\'utilisateur {{user}}',
    'previous_punishments' => 'Sanctions antérieures',
    'no_previous_punishments' => 'Aucune sanction antérieure',
    'reset_avatar' => 'Reset Avatar',
    'warning' => 'Alerte',
    'ip_ban' => 'IP Ban',
    'reason' => 'Raison',
    'warn_user' => 'Avertir(warn) l\'utilisateur',
    'ban_user' => 'Bannir l\'utilisateur',
    'enter_valid_punishment_reason' => 'Veuillez saisir une raison valable de votre sanction, entre 5 et 5000 caractères.',
    'user_punished' => 'Utilisateur sanctionné.',
    'user_punished_alert' => '{{staffUser}} a sanctionné {{user}}',
    'revoke' => 'Révoquer',
    'revoked' => 'Révoqué',
    'acknowledged' => 'Remerciements',
    'confirm_revoke_warning' => 'Voulez-vous vraiment révoquer cet avertissement ?',
    'confirm_revoke_ban' => 'Êtes-vous sûr de vouloir révoquer cette sanction? L’utilisateur ne sera plus banni, même s’il a un bannissement plus récent !',
    'punishment_revoked' => 'Sanction révoquée.',
    'cant_punish_admin' => 'Vous ne pouvez pas sanctionner un administrateur !',
    'viewing_all_punishments' => 'Voir toutes les sanctions',
    'no_punishments_found' => 'Aucune sanction n\'a été trouvée.',
    'view_user' => 'Voir l\'utilisateur',
    'when' => 'Quand',
    'staff' => 'Staff',
    'type' => 'Type',
    'recent_punishments' => 'Sanctions récentes',
    'created' => 'Créé:',
    'staff:' => 'Staff:',
    'reason:' => 'Raison:',

    // Users
    'recent_registrations' => 'Enregistrements récents',
    'reset_profile_banner' => 'Réinitialiser la bannière du profil'

];
