...
class ProfileUtils {
    ...
    private static function getUUIDFromUsername(string $username): ?array {
        ...
        if (!$result->hasError() && $result->getStatus() === 200) {
            $response = json_decode($result->contents(), true);
     ...
     *
     * @return string Properly formatted UUID (According to UUID v4 Standards xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx
     * WHERE y = 8,9,A,or B and x = random digits.)
     */
