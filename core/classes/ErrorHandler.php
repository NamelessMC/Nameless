...
class ErrorHandler {
    ...
    private static function parseFrame($exception, $frame_file, $frame_line, $number = null) {
        ...
        return [
            ...
            'file' => $frame_file,
            'line' => $frame_line,
            'start_line' => (is_array($lines) && count($lines) >= self::LINE_BUFFER && $frame_file >= self::LINE_BUFFER) ? ($frame_line - self::LINE_BUFFER) : 1,
            'highlight_line' => (is_array($lines) && count($lines) >= self::LINE_BUFFER && $frame_line >= self::LINE_BUFFER) ? (self::LINE_BUFFER + 1) : $frame_line,
