...
class NamelessOAuth extends Instanceable {
    ...
    public function isEnabled(string $provider): bool {
        return self::$_db->get('oauth', ['provider', $provider])->first()->enabled === '1';
    }
    ...
    public function getCredentials(string $provider): array {
        $data = self::$_db->get('oauth', ['provider', $provider])->first();
        return [
            $data->client_id,
            $data->client_secret,
        ];
    ...
    public function setEnabled(string $provider, int $enabled): void {
        self::$_db->query("UPDATE nl2_oauth SET enabled = ? WHERE provider = ?", [$enabled, $provider]);
    }
     ...
     *
     * @return ?string The NamelessMC user ID of the user linked to the provider.
     */
    public function getUserIdFromProviderId(string $provider, string $provider_id): ?string {
        return self::$_db->query(
            'SELECT user_id FROM nl2_oauth_users WHERE provider = ? AND provider_id = ?',
            [$provider, $provider_id]
        )->first()->user_id;
