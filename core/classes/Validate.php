...
class Validate {
                    ...
                    switch ($rule) {
                        case Validate::MIN;
                            if (mb_strlen($value) < $rule_value) {
                                $this->addError($this->getError($item, "{$item} must be a minimum of {$rule_value} characters."));
                            }
                            break;

                        case Validate::MAX;
                            if (mb_strlen($value) > $rule_value) {
                                $this->addError($this->getError($item, "{$item} must be a maximum of {$rule_value} characters."));
                            }
                            break;

                        case Validate::MATCHES;
                            if ($value != $source[$rule_value]) {
                                $this->addError($this->getError($item, "{$rule_value} must match {$item}."));
                            }
                            break;

                        case Validate::AGREE;
                            if ($value != 1) {
                                $this->addError($this->getError($item, "You must agree to our terms and conditions in order to register."));
                            }
                            break;
                        ...
                        case Validate::UNIQUE;
                            ...
                            if ($check->count()) {
                                $this->addError($this->getError($item, "The username/email {$item} already exists!"));
                            }
                            break;
                        ...
                        */
                        ...
                        case Validate::EMAIL;
                            if (!filter_var($value, FILTER_VALIDATE_EMAIL)) {
                                $this->addError($this->getError($item, "{$value} is not a valid email."));
                            }
                            break;

                        case Validate::TIMEZONE:
                            if (!in_array($value, DateTimeZone::listIdentifiers(DateTimeZone::ALL))) {
                                $this->addError($this->getError($item, "The timezone {$item} is invalid."));
                            }
                            break;
                        ...
                        case Validate::IS_ACTIVE;
                            ...
                            if ($check->count()) {
                                ...
                                if ($isuseractive == 0) {
                                    $this->addError($this->getError($item, "That username is inactive. Have you validated your account or requested a password reset?"));
                                }
                            }
                            break;
                        ...
                        case Validate::IS_BANNED;
                            ...
                            if ($check->count()) {
                                ...
                                if ($isuserbanned == 1) {
                                    $this->addError($this->getError($item, "The username {$item} is banned."));
                                }
                            }
                            break;
                        ...
                        case Validate::ALPHANUMERIC:
                            if (!ctype_alnum($value)) {
                                $this->addError($this->getError($item, "{$item} must be alphanumeric."));
                            }
                            break;

                        case Validate::NUMERIC:
                            if (!is_numeric($value)) {
                                $this->addError($this->getError($item, "{$item} must be numeric."));
                            }
                            break;
                    }
                }
     ...
     */
    private function getError($field, $fallback) {
