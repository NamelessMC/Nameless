...
try {
    DB::getInstance()->addColumn('mc_servers', '`show_ip`', "tinyint(1) NOT NULL DEFAULT '1'");
} catch (Exception $e) {
    echo $e->getMessage() . '<br />';
}
...
try {
    DB::getInstance()->addColumn('users', '`avatar_updated`', "int(11) DEFAULT NULL");
} catch (Exception $e) {
    echo $e->getMessage() . '<br />';
}
...
// Online guests
try {
    DB::getInstance()->createTable("online_guests", " `id` int(11) NOT NULL AUTO_INCREMENT, `ip` varchar(45) NOT NULL, `last_seen` int(11) NOT NULL, PRIMARY KEY (`id`)", "ENGINE=$db_engine DEFAULT CHARSET=$db_charset");
} catch (Exception $e) {
...
// Login reCAPTCHA
$recaptcha_check = DB::getInstance()->get('settings', array('name', 'recaptcha_login'))->results();

if (!count($recaptcha_check)) {
    DB::getInstance()->insert('settings', array('name' => 'recaptcha_login', 'value' => 'false'));
}
