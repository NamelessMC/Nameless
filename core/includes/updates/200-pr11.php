...
try {
    $placeholders_exist = DB::getInstance()->get('settings', array('name', 'placeholders'))->results();
    if (!count($placeholders_exist)) {
        $placeholders = DB::getInstance()->get('placeholders_settings', array('id', '<>', '0'))->results();
