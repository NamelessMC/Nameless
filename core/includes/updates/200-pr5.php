...
// Topic placeholders
try {
    DB::getInstance()->addColumn('forums', '`topic_placeholder`', 'mediumtext');
} catch (Exception $e) {
    echo $e->getMessage() . '<br />';
}
...
// Panel templates
try {
    DB::getInstance()->createTable("panel_templates", " `id` int(11) NOT NULL AUTO_INCREMENT, `name` varchar(64) NOT NULL, `enabled` tinyint(1) NOT NULL DEFAULT '0', `is_default` tinyint(1) NOT NULL DEFAULT '0', PRIMARY KEY (`id`)", "ENGINE=$db_engine DEFAULT CHARSET=$db_charset");
} catch (Exception $e) {
    echo $e->getMessage() . '<br />';
}
...
// Permissions
$existing_user_permissions = DB::getInstance()->get('groups', array('id', 1))->results();
...
$existing_mod_permissions = DB::getInstance()->get('groups', array('id', 3))->results();
...
$existing_admin_permissions = DB::getInstance()->get('groups', array('id', 2))->results();
...
// Revamp template
try {
    $revamp_template_exists = DB::getInstance()->get('templates', array('name', 'DefaultRevamp'))->results();
