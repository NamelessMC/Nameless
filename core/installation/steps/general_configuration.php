    ...
    <div class="ui segments">
        <div class="ui secondary segment">
            <h4 class="ui header">
                ...
                echo $language->get('installer', 'general_configuration'); ?>
            </h4>
        </div>
            ...
            <div class="ui centered grid">
                <div class="sixteen wide mobile twelve wide tablet ten wide computer column">
                    <div class="ui form">
                        ...
                        if (getenv('NAMELESS_HOSTNAME_HIDE') !== false) {
                            echo 'style="display: none"';
                        } ?>>
                            <?php

                            create_field('text', $language->get('installer', 'host'), 'hostname', 'inputHostname', getenv('NAMELESS_HOSTNAME') ?: Output::getClean($_SERVER['SERVER_NAME'])); ?>
                            <p><?php

                                echo $language->get('installer', 'host_help'); ?></p>
                            <div class="ui divider"></div>
                        </div>
                        <div <?php

                        if (getenv('NAMELESS_FRIENDLY_URLS_HIDE') !== false) {
                            echo 'style="display: none"';
                        } ?>>
                            <?php

                            create_field('select', $language->get('installer', 'friendly_urls'), 'friendly', 'inputFriendly', getenv('NAMELESS_FRIENDLY_URLS') ?: 'false', [
                                'true' => $language->get('installer', 'enabled'),
                                'false' => $language->get('installer', 'disabled'),
                            ]); ?>
                            <p><?php

                                echo $language->get('installer', 'friendly_urls_info'); ?></p>
                        </div>
                    </div>
                </div>
            </div>
